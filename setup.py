                ...
                if handler == "console":
                    ...
                    if venv_check() is True:
                        value["filename"] = os.path.join(
                            sys.prefix, "var/logs/jsnapy/jsnapy.log"
                        )
        ...
        if dir_path != "/etc/jsnapy":
            ...
            if venv_check() is True:
                default_config_location = os.path.join(
                    sys.prefix, "etc", "jsnapy", "jsnapy.cfg"
                )
            elif "win32" in sys.platform:
                default_config_location = os.path.join(
                    expanduser("~"), "jsnapy", "jsnapy.cfg"
                )
            else:
...
log_files = [os.path.join("logs", j) for j in os.listdir("logs")]
