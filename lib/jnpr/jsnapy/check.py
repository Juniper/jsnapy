...
class Comparator:
    ...
    def get_xml_reply(self, db, snap):
        ...
        ##### sometimes snapshot files are empty, when cmd/rpc reply do not contain any value
            ...
            self.logger_check.error(
                colorama.Fore.RED + "ERROR, Snapshot file is empty !!",
                extra=self.log_detail,
            )
            return
        else:
    ...
    def expression_evaluator(
        ...
        if ele is not None:
            ...
            if obj:
                ...
                if obj.group(3):
                    ...
                    ele_list = ele_list + [
                        elements.strip()
                        for elements in xml_paras.split(",")
                        if elements.strip() != ""
                    ]
            else:
        ...
        # with --check ####
        is_skipped = False
        if testop in ["no-diff", "list-not-less", "list-not-more", "delta"]:
    ...
    def expression_builder(self, sub_expr, parent_op=None, **kwargs):
        ...
        # perform validation
        if parent_op and (
            (len(sub_expr) > 1 and self.is_unary_op(parent_op))
            or (len(sub_expr) < 2 and self.is_binary_op(parent_op))
        ):
            self.logger_check.info(
                colorama.Fore.RED + "ERROR!!! Malformed sub-expression",
                extra=self.log_detail,
            )
            return
        for elem in sub_expr:
            ...
            if len(op_list) == 1:
                ...
                # for skipping cases
                if res is None or (
                    last_test_instance["count"]["pass"] == 0
                    and last_test_instance["count"]["fail"] == 0
                    and testop not in ["no-diff", "list-not-less", "list-not-more"]
                ):
                    continue
    ...
    def compare_reply(
        ...
        if not len(tests) and (check is True or action == "check"):
            ...
            for test in tests:
                ...
                if (
                    final_boolean_expr == ""
                    or final_boolean_expr is None
                    or final_boolean_expr == str(None)
                ):
                    continue
    ...
    def compare_diff(self, pre_snap_file, post_snap_file, check_from_sqlite):
        ...
        if check_from_sqlite:
            ...
            if os.path.isfile(pre_snap_file) and os.path.isfile(post_snap_file):
                ...
                self.logger_check.info(
                    colorama.Fore.RED + "ERROR!!! Files are not present in given path",
                    extra=self.log_detail,
                )
    ...
    def compare_xml(self, op, db, teston, pre_snap_value, post_snap_value):
        ...
        try:
            ...
            self.logger_check.error(
                colorama.Fore.RED + "Error!! from pre or post snap file: %s" % ex,
                extra=self.log_detail,
            )
        else:
            result = []
            ...
            if pre_root is not None and post_root is not None:
                ...
                self.logger_check.info(
                    colorama.Fore.BLUE + "Difference in pre and post snap file",
                    extra=self.log_detail,
                )
                flag = tres["result"]
            else:
            ...
            if len(result) == 0 and flag is True:
                self.logger_check.info(
                    colorama.Fore.BLUE + "    No difference   ", extra=self.log_detail
                )
                self.logger_check.info(
                    colorama.Fore.GREEN
                ...
                for index, res in enumerate(result):
                    self.logger_check.info(
                        colorama.Fore.RED + str(index) + "] " + res,
                        extra=self.log_detail,
                    )
    ...
    def generate_test_files(
        ...
        if main_file.get("tests") is None:
            ...
            for tests in tests_files:
                ...
                self.logger_check.info(
                    colorama.Fore.BLUE + message, extra=self.log_detail
                )
                for val in tests_included:
                    self.logger_check.info(
                        "Tests Included: %s " % (val), extra=self.log_detail
                    )
                    ...
                    description_index = [
                        index[0]
                        for index in enumerate(tests[val])
                    ...
                    try:
                        if any("command" in d for d in tests[val]):
                            index = next(
                                (i for i, x in enumerate(tests[val]) if "command" in x),
                                0,
                            )
                            command = (
                                tests[val][index].get("command").split("|")[0].strip()
                            )
                            ...
                            index = next(
                                (i for i, x in enumerate(tests[val]) if "rpc" in x), 0
                            )
                            index_kwargs = next(
                                (
                                    i
                                    for i, x in enumerate(tests[val])
                            ...
                            if (
                                "kwargs" in tests[val][index_kwargs]
                                and tests[val][index_kwargs].get("kwargs") is None
                            ):
                                del tests[val][index_kwargs]["kwargs"]

                            if (
                                "args" in tests[val][index_kwargs]
                                and tests[val][index_kwargs].get("args") is None
                            ):
                                del tests[val][index_kwargs]["args"]

                            if (
                                "kwargs" in tests[val][index_kwargs]
                                or "args" in tests[val][index_kwargs]
                            ):
                                ...
                                if (
                                    action == "check"
                                    and pre_user is not None
                                    and post_user is not None
                                ):
                        ...
                        self.logger_check.error(
                            colorama.Fore.RED + "ERROR Occurred: %s" % str(ex),
                            extra=self.log_detail,
                        )
                    else:
                        # extract snap files, if check from sqlite is true t
                        if db.get("check_from_sqlite") is True and (
                            check is True or diff is True or action in ["check", "diff"]
                        ):
                            ...
                            if (
                                reply_format != data_format1
                                or reply_format != data_format2
                            ):
                                self.logger_check.error(
                                    colorama.Fore.RED
                        ...
                        # snapshots ####
                        if (
                            check is True or action == "check"
                        ) and reply_format == "xml":
                        ...
                        # two snapshots word by word ####
                            ...
                            self.compare_diff(
                                snapfile1, snapfile2, db.get("check_from_sqlite")
                            )
