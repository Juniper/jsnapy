...
class Parser:
        ...
        if isinstance(rpc_reply, bool) and format == "text":
            ...
            if len(err):
                ...
                with open(output_file, 'w') as f:
                    ...
                    ##### displaying rpc reply in debug logs
                    #self.logger_snap.debug(
                    #    colorama.Fore.BLUE +
                    #    etree.tostring(rpc_reply), extra = self.log_detail)
    ...
    def run_cmd(self, test_file, t, formats, dev, output_file, hostname, db ):
        ...
        try:
            self.logger_snap.info(colorama.Fore.BLUE +"Taking snapshot for %s ................" %command, extra = self.log_detail)
            rpc_reply_command = dev.rpc.cli(command, format=cmd_format)
        except Exception:
            self.logger_snap.error(colorama.Fore.RED + "ERROR occurred %s" % str(sys.exc_info()[0]), extra = self.log_detail)
            ...
            if db['store_in_sqlite'] is True and self._check_reply(rpc_reply_command, cmd_format):
                self.store_in_sqlite(db, hostname, cmd_name, cmd_format, rpc_reply_command, output_file)
    ...
    def run_rpc(self, test_file, t, formats, dev, output_file, hostname, db):
        ...
        if db['store_in_sqlite'] is True and self._check_reply(rpc_reply, reply_format):
            self.store_in_sqlite(db, hostname, rpc, reply_format, rpc_reply, output_file)
