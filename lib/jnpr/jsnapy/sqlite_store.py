...
class JsnapSqlite:
    ...
    def insert_data(self, db):
        ...
        with sqlite3.connect(self.db_filename) as con:
            con.execute(
                """update '%s' set id = id + 1 where cli_command = :cli"""
                % self.table_name,
                {"cli": db["cli_command"]},
            )
            con.execute(
                """delete from '%s' where id>49 AND cli_command = :cli"""
                % self.table_name,
                {"cli": db["cli_command"]},
            )
            con.execute(
                """insert into '%s' (id, filename, cli_command, snap_name, data_format, data) values (0, :file,
                         :cli, :snap, :format, :xml)"""
                % self.table_name,
                {
                    "file": db["filename"],
