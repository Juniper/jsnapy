    ...
    def __init__(self, db_name):
        ...
        if not os.path.isfile(self.db_filename):
            self.logger_sqlite.error(
                colorama.Fore.RED + "Database %s does not exist." % db_name,
    ...
    def get_xml_using_snapname(self, hostname, command_name, snap_name):
        ...
        with sqlite3.connect(self.db_filename) as con:
            try:
                ...
                cursor.execute(
                    "SELECT MIN(id), data_format, data FROM '%s' WHERE snap_name = :snap AND cli_command = :cli"
                    % table_name,
                ...
                if not row:
                    raise Exception(
                        ...
                        % (snap_name, command_name.replace("_", " "))
                    )
                ...
                self.logger_sqlite.error(
                    colorama.Fore.RED + "ERROR!! Complete message is %s" % ex,
                    extra=self.sqlite_logs,
                )
            else:
    ...
    def get_xml_using_snap_id(self, hostname, command_name, snap_id):
        ...
        with sqlite3.connect(self.db_filename) as con:
            try:
                ...
                cursor.execute(
                    "SELECT id, data_format, data FROM '%s' WHERE id = :id AND cli_command = :cli"
                    % table_name,
                ...
                if not row:
                    raise Exception(
                        ...
                        % (snap_id, command_name.replace("_", " "))
                    )
                idd, data_format, data = row
                if data is None:
                    raise Exception(
