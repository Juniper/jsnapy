...
class Notification:
    ...
    def notify(self, mail_file, hostname, password, test_obj):
        ...
        self.logger_notify.debug(
            colorama.Fore.BLUE + "Sending mail............", extra=self.log_details
        )
        ...
        outputText = template.render(
            device=hostname,
            name=mail_file["recipient_name"],
            tests=testdetails,
            date=time.ctime(),
            tpassed=test_obj.no_passed,
            tfailed=test_obj.no_failed,
            fresult=test_obj.result,
            sname=mail_file["sender_name"],
        )
        msg = MIMEMultipart()
        ...
        try:
            ...
            self.logger_notify.error(
                colorama.Fore.RED + "ERROR!!  in sending mail: %s" % ex,
                extra=self.log_details,
            )
        server.quit()
